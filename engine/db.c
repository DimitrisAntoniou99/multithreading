...
//---------------------------------------------PROSTHETOS CODE--------------------------------------------------
//prototypa gia ta mutex
pthread_mutex_t bd_add_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t bd_gets_mutex = PTHREAD_MUTEX_INITIALIZER;
...
{
    pthread_mutex_lock(&bd_add_mutex );				//kleidaria gia to add (lock)
    ...
    pthread_mutex_unlock(&bd_add_mutex ); 			//kleidaria gia to add (unlock)
    return memtable_add(self->memtable, key, value);
}

int db_get(DB* self, Variant* key, Variant* value)
{
    pthread_mutex_lock(&bd_gets_mutex); 			//kleidaria gia to get (lock)
    if (memtable_get(self->memtable->list, key, value) == 1)
        return 1;
    pthread_mutex_unlock(&bd_gets_mutex);			//kleidaria gia to get (unlock)
